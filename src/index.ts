...
const run = async () => {
	...
	try {
		...
		await zx.$`cd ${path.join(backflowPath, 'artifact')}`
		...
		const pkgJSON = JSON.parse(await fs.readFile(path.join(backflowPath, 'artifact', 'package.json'), { encoding: 'utf-8' }))
		pkgJSON.version = input.contract.version
		await fs.writeFile(path.join(backflowPath, 'artifact', 'package.json'), JSON.stringify(pkgJSON, null, 2))
		console.log('[PUBLISHER] Update package.json version')

		// Change package-lock.json version
		const pkgLockJSON = JSON.parse(await fs.readFile(path.join(backflowPath, 'artifact', 'package-lock.json'), { encoding: 'utf-8' }))
		pkgLockJSON.version = input.contract.version
		await fs.writeFile(path.join(backflowPath, 'artifact', 'package-lock.json'), JSON.stringify(pkgLockJSON, null, 2))
